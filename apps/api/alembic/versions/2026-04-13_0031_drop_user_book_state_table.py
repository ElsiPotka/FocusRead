...
def upgrade() -> None:
    op.drop_index("ix_user_book_state_user_last_opened", table_name="user_book_state")
...
def downgrade() -> None:
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("user_id", "book_id", name=op.f("pk_user_book_state")),
    )
    op.create_index(
