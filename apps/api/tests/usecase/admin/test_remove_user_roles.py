...
def _roles() -> list[Role]:
    return [
        Role.create(name=RoleName.CLIENT, description="Client access"),
    ]


class TestRemoveUserRoles:
    async def test_removes_roles_and_returns_remaining_roles(self, uow):
        user_id = UserId(uuid4())
        roles = _roles()
        uow.users.get.return_value = _user(user_id)
        uow.roles.get_by_names.return_value = roles
        uow.roles.list_for_user.return_value = []

        result = await RemoveUserRoles(uow).execute(
            user_id=user_id.value,
            role_names=[RoleName.CLIENT],
        )

        assert result == []
        uow.roles.remove_many_from_user.assert_called_once_with(
            user_id,
    ...
    async def test_raises_when_user_is_missing(self, uow):
        ...
        with pytest.raises(NotFoundError):
            await RemoveUserRoles(uow).execute(
                user_id=user_id,
                role_names=[RoleName.CLIENT],
            )

        uow.roles.remove_many_from_user.assert_not_called()
