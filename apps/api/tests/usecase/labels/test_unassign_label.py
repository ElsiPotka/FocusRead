...
async def test_unassign_label(uow, user_id, book, label, book_repo, label_repo):
    ...
    label_repo.unassign_from_book.assert_awaited_once()
...
async def test_unassign_book_not_found(uow, book_repo):
    ...
    with pytest.raises(NotFoundError, match="Book not found"):
        await use_case.execute(
            book_id=uuid4(), label_id=uuid4(), user_id=uuid4()
        )
