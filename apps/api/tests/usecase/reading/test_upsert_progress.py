...
@pytest.fixture
def uow(book_repo, session_repo, stat_repo):
...
async def test_creates_new_session_when_none_exists(
    uow, book_repo, session_repo, stat_repo, cache, book
):
...
async def test_updates_existing_session(
    ...
    existing = ReadingSession.create(user_id=book.owner_user_id, book_id=book.id)
    book_repo.get_for_owner.return_value = book
